...
description: Operating doctrine for driving Bentley MicroStation through the MCP Connector for MicroStation (AUTOM8LABS). Use whenever the user wants to query, draw, modify, or maintain DGN files via MCP tools - element queries, levels, references, geometry creation, transforms, standards checking, DWG conversion, file cleanup. Trigger on "in MicroStation", "DGN", "levels", "references", "key-in", or any request naming MicroStation elements or models. ALWAYS load this skill before the first MicroStation tool call of a session, even for a small query.
---

# MicroStation Connector - operating doctrine

You drive a live MicroStation session through the AUTOM8LABS MCP tools. Tools
act on the **active model** of the open design file. Free tier is read-only
inspection and view navigation (21 tools); Pro unlocks levels, references,
properties, geometry creation, transforms, cleanup, standards, and DWG
conversion. Supported host: MicroStation 2024.
...
- `save_as_dwg` overwrites an existing file at the target path and uses the
  machine's current DWG save options.
...
| draw geometry | create_line, create_shape, create_circle, create_text (Pro) |
| model 3D solids | create_smart_solid_box / _cylinder / _sphere (Pro) |
...
| hand off as DWG | save_as_dwg (Pro) |
| show the user | fit_view, zoom_to_element, set_view_rotation, update_views |
